...
*/
(function () {
    var SRC = "C:/pprolab/m6_build.prproj";
    var DIR = "C:/pprolab/frames6";
    var CLONE = "차명12_인트로_레이어";
    var TICKS = 254016000000;

    /*  컷 경계 직후·중간·직전을 고루 본다. 모션이 걸린 구간이라 값이 변한다  */
    var SHOTS = [3.45, 4.60, 6.60, 7.50, 10.00, 12.30, 13.20, 13.70, 14.40, 15.45];

    var out = [];
    function say(k, v) { out.push(k + "\t" + v); }
    function probe(k, fn) { try { say(k, String(fn())); } catch (e) { say(k + "_ERR", e.toString()); } }
    function done(m) {
        var f = new File("C:/pprolab/m6_frames.txt");
    ...
    var w = 0;
    while (w < 180 && !(app.project && app.project.path.indexOf("m6_build") >= 0
    ...
    return done("m6_frames " + (okCnt > 0 ? "ok" : "FAILED"));
})();
