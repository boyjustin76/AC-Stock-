# -*- coding: utf-8 -*-
"""레거시 (2026-09-17 격리 — 실행 금지). 옛 **한지판** 롤링 광고 전용 검사기.

한지판은 09-17 에 지웠고, 납품 jpg 는 이제 사람이 손본 롤링광고_사용자수정.ai 에서 뽑는다 —
밝은 바탕·어두운 글자를 전제한 이 검사기는 옻칠판에 헛값을 낸다. OUT 경로도 박혀 있었다(총괄 개선안 B-4, D 파일).
측정 요령(섬유 vs 글자는 색이 아니라 모양으로 가린다)은 brand/EXTENDSCRIPT-TRAPS.md 원칙 절과 아래 원래 머리말에 남긴다.

원래 머리말: 롤링 광고 배너 검사 — 본문 글자 오른쪽 끝 ↔ CTA 현판 왼쪽 바깥 변 간격.
