/*  AE 작업실 폴더를 찾아준다 (labdir.py 의 Node 판).
    ...
    찾는 순서: AELAB_DIR 환경변수 → config.json 의 labDir → 위로 올라가며 폴더 이름 찾기. 못 찾으면 멈춘다 (2026-09-17)
*/
...
export function labDir() {
  for (const v of [process.env.AELAB_DIR, fromConfig(), byWalkingUp()]) {
    if (v && existsSync(v)) return v.split(String.fromCharCode(92)).join('/');
  }
  throw new Error(`AE 작업실 폴더 ${FOLDER} 를 못 찾았다 — 환경변수 AELAB_DIR 을 주거나 config.json 의 labDir 에 적는다 (찾기 시작: ${HERE})`);
}
