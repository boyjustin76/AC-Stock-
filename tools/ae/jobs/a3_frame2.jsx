/*  saveFrameToPng 가 조용히 아무것도 안 만든다. 표기법을 바꿔 가며 잰다.
    ...
    ⚠ 2026-09-16 정정 — **이 잡의 판정은 틀렸다.**
    로그에는 ⑥(이미 열어 둔 File)만 성공한 것으로 남았지만, 나중에 디스크를 보니
    v1.png · v2 · v3.png · v4.png 가 **전부 52,792 바이트로 멀쩡히 있었다.**
    ①~④ 도 다 성공했고, "새 파일=없음" 은 쓰기 직후에 확인해서 난 거짓 실패였다.
    ⑥ 만 통과한 것처럼 보인 이유는 f.open("w")/close() 로 **미리 만들어 둔 0바이트 파일이
    그 시점에 이미 존재**했기 때문이다. 표기법 문제가 아니라 **타이밍 문제**였다.

    그래서 진짜 규칙은 이렇다.
      · 문자열이 아니라 File 객체를 준다 (⑤ 만 진짜 실패 — "파일 또는 폴더 개체가 아니다")
      · 확장자를 함수가 붙여 주지 않는다 (② 는 확장자 없는 'v2' 로 생겼다)
      · 쓰기가 곧바로 안 보인다 — $.sleep(600) 뒤 File 을 **다시 만들어** 확인한다 (200 은 모자랐다)
      · 투명도 플래그 같은 것은 없다. 알파는 그대로 보존된다 (검사 캡처 anim_*.png 알파 최소 0)  */
