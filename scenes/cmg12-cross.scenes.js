 ...
 * 프리셋의 0~16초는 빈 자리가 아니라 **고정 오프닝**이다:
 *
 *      0.00~ 3.50   훅 스틸 + '댓글 유도' 그래픽
 *      5.93~12.20   인트로 애니메이션(지그재그 + 매수/매도 태그) → 타이틀 카드
 *     12.20~14.73   비어 있음
 *     14.73~        본문 시작
 *
 * 5.93~12.20 은 프리셋 애니메이션이 화면의 주인공이다. 여기에 라벨 붙은 차트를
 * 깔았더니 매수/매도 태그가 두 벌로 겹치고 글자가 서로를 파묻었다(실측 프레임 09_00s).
 * 그래서 그 구간의 차트는 **라벨을 전부 빼고 배경으로만** 깐다.
 * 프리셋이 비는 12.20 부터 다시 차트가 이야기를 한다.
 *
 ...
 * 여기서 하는 zoom 은 픽셀 확대가 아니라 **데이터 재프레이밍**이라 성격이 다르다 —
 * 컷4 에서만 쓴다.
 *
