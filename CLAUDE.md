...
| | | | **2.5 썸네일 → 로컬 클로드** | |
| **숏폼** | E 세션 (규칙·지시서·검사는 저장소) | 외부 | **← 이 저장소** (STT 컷편집·srt, `tools/cutedit`) | **← 이 저장소** (1:1 1080²/30fps, `scenes/sl-*.scenes.js`) |
...
3. **`brand/EDIT-RULEBOOK.md` — 연출 룰북.** 피드백에서 확정된 규칙 12개. 컷을 짜기 전에 읽는다.
