...
SELECT * FROM thumbnail_rule;  -- 롱폼 썸네일 규격 (템플릿 실측)
SELECT * FROM shortform_srt;   -- 숏폼 자막 실측 (길이·초당 글자수)
...
- **숏폼 목표 길이는 45초 = 307자다.** 초당 6.82자(자막 13편 실측).
  훅 26자·CTA 26자는 고정이니 **줄일 때는 본문에서만 줄인다.**
  나간 편들은 중앙값 55.9초로 목표보다 24% 길다 — 실태를 표준으로 착각하지 않는다.
- **자막(.srt)은 각 숏폼 폴더의 `소스+원본` 안에 있다.** 25개 중 14개.
- **회사 전체 드라이브 루트는 `1JfQCjJgMwHzyq2mpSu-OoiDEIiBpyUXH`** 다. 여기서 다 내려간다.
- **썸네일 타이틀은 노랑(메인)+흰색(서브), 둘 다 GmarketSansBold·검정 외곽선·자간 −40.**
  글자 수가 달라도 폭을 맞춘다 (윗줄 1120px, 아랫줄 1185px). `tools/thumbnail.py` 가 계산한다.
- **썸네일 태그(매수·익절)는 그리지 않는다.** 템플릿 안 진짜 버튼 픽셀을 뜯어 쓴다
  (`brand/thumbnail/btn_*.png`). 코드로 흉내 내면 모서리·화살촉·자간이 미묘하게 어긋난다.
  익절은 매수 버튼을 좌우 반전한 모양에 Color Overlay `#00FF24` 가 걸린 것이다.
- **psd-tools 는 `composite()` 가 빈 그림을 주는 레이어가 많다.** `topil()` 은 원본 픽셀을 준다.
  효과까지 필요하면 그룹을 `solo()` 로 켜고 composite — **원본 회차 그룹은 꺼져 있어서
  복제본도 꺼진 채로 나온다.**
- **썸네일 납품은 `.png` 다.** `tools/thumbnail_png.py` — 차트 한 장(배경 투명), 완성본 한 장.
  차트는 `--format alpha --stills 1` 로 배경 없이 뽑고, 버튼 자리는 씬 파일의 `probe` 컷에서
  자홍·청록을 찾아 읽는다. 짐작해서 좌표를 넣지 않는다.
  `.psd` 는 포토샵이 두 번 거부해서 접었다.
- **썸네일 `.psd` 는 템플릿을 편집해서 만든다** — `tools/psdedit.py`.
  회차 그룹을 통째로 복제하고 그 안의 차트·타이틀만 바꾼다. 새로 쓰면 그룹·스마트오브젝트·
  조정레이어·효과·라이브 텍스트가 다 날아간다. 세 가지를 꼭 지킨다:
  **① 복제할 때 `lyid` 를 새로 매긴다** (안 그러면 합성에서 통째로 빠진다),
  **② 한글 레이어명은 cp949→mac_roman 로 넣는다** (pascal 칸이 macroman 이라),
  **③ 안 쓰는 회차 그룹은 들어내고 차트는 RLE 로 압축한다** (180MB → 25.5MB),
  **④ 글자를 바꿀 때 EngineData 의 `RunArray` 와 `RunLengthArray` 를 함께 줄인다** —
  개수가 어긋나면 포토샵이 "프로그램 오류로 인하여 열 수 없습니다" 로 파일을 거부한다.
  `Template.check()` 가 저장 전에 잡는다. **check 를 끄고 저장하지 않는다.**
- `tools/psdwrite.py` 는 PSD 를 처음부터 쓰는 쪽. 지금은 안 쓰지만 남겨 뒀다.
- **psd-tools 는 아트보드 문서를 `composite()` 로 못 그린다** (원본 템플릿도 마찬가지).
  회차 그룹만 `composite(force=True)` 해서 캔버스에 얹어 미리보기를 만든다.
- **텍스트를 바꾸면 픽셀도 같이 구워야 한다.** 레이어에 래스터가 캐시돼 있어서
  `set_text` 만 하면 포토샵도 옛 글자를 보여 준다. `Template.bake_text()` 가 둘을 함께 바꾼다.
