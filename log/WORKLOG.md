...
| 컷 경계에서 요소를 지우면 깜빡임이다 — 대체 전까지 유지한다 | 컷을 새로 시작할 때 앞 컷의 라벨·태그·원을 layers 에 안 넣으면 화면에서 뚝 사라졌다 다음 컷에서 다시 튀어나온다. 차12 인트로 컷2를 '프리셋 타이틀이 덮는다'는 가정으로 비웠다가 반려됨 — 실제 편집본에선 차트가 그대로 보였다 (2026-09-01 이정찬). 화면을 덮는 그래픽 가정은 실측 전엔 믿지 마라 | 컷을 나누는 기준은 '새 요소로 대체할 때'. 앞 컷 요소를 still()(in [-1,0]·drawDur 0·popDur 0·growDur 0)로 전부 이월하고, 교체는 같은 자리 크로스페이드, 퇴장은 카메라 이동으로만(clamp:false·toBar). 규칙 전문은 brand/EDIT-RULEBOOK.md ⑧~⑩ |
...
29. **차12 훅 컷4~6 대본 해석 결정 대기** — '①곧바로 하락(가짜 신호) 손실 거나 ②추세 지속 중 늦은 지표에 손실 전환' — 총괄은 별개 두 사례로 읽음(근거: '~보거나' 병렬, ②의 '익절할지 몰라 손실 전환'은 보유 포지션이 필요, 기획서가 가짜 신호를 '지표가 쏟아내는 잘못된 신호'로 정의). 이정찬 대안 해석: 하락이 가짜고 그 뒤 큰 상승이 진짜, 그걸 놓친 게 손실. 전자로 확정되면 컷6 랠리를 더 키워 '수익→손실 전환'이 또렷하게(이어붙인 trend↑ strength 1.3을 올리고 find-events 재실측), 후자면 컷5~6을 재구성(cmgMissed '놓친 구간' 활용). 룰북 '미결' 항목과 같이 갱신할 것  _(대기: 이정찬 답변 후)_
