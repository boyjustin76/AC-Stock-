...
| 타이틀 크기는 사람이 정하지 않는다 — 상자가 정한다 | 차12 썸네일 3안이 다 '좌상단 타이틀이 너무 크다'로 반려 (2026-09-03 이정찬). 규격은 지켰다 — 규칙 4·5 의 고정 높이(윗줄 141 · 아랫줄 194)를 그대로 썼다. 문제는 그게 열 회차 완성본에서 재 온 '수치'라는 것이다. 차12 는 문구가 길어서 같은 높이인데 폭이 1488px 까지 나갔고 익절 태그와 붙었다. 사용자 지시: 이전 썸네일의 크기를 정량적으로 따르지 말고 맥락(격자박스)으로 파악해라 | 상한선을 하나 얹었다 — 상자를 먼저 정하고(config.json titleBox), 넓은 줄이 상자 폭을 넘을 때만 두 줄을 같은 배율로 줄인다. 안 넘으면 예전 회차와 똑같은 고정 크기다. 확대는 안 한다 — 폭에 맞춰 항상 역산하던 옛 컨테이너 방식은 짧은 문구를 거대하게 만든다. 고정 높이 규칙(4·5)을 버린 게 아니라 그 위에 걸어 둔 것이다. 규칙 28 |
...
**59. 썸네일 3안 다 좌상단 타이틀이 너무 크다. 비율 15~20% 줄여라. 그리고 앞으로는 이전 썸네일의 글자 크기(수치)를 그대로 따르지 말고 맥락(격자박스)으로 파악해서 상자 안에 딱 들어가게 해라 (영상 고정 양식의 좌상단·우상단·자막 상자를 안내선으로 실측한 스샷 4장 첨부)**
