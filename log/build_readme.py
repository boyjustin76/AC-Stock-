...
def build() -> str:
    ...
    a("## 이 저장소가 맡는 곳")
    a("")
    a("영상 한 편은 네 단계를 거칩니다. 이 저장소는 그중 **롱폼 3단계(모션그래픽)** 와")
    a("**숏폼 1단계(대본)** 를 맡습니다. 나머지는 사람이 합니다.")
    a("")
    a("```mermaid")
    a("flowchart LR")
    a('  subgraph L ["롱폼 — 작업중"]')
    ...
    a('  subgraph S ["숏폼"]')
    a("    direction LR")
    a('    S1["<b>1. 대본</b><br/><small>규칙·지시서·검사</small>"]')
    a('    S1 --> S15["1.5 녹음"] --> S2["2. 컷편집 · 자막"] --> S3["3. 모션그래픽 · 소스"]')
    a("  end")
    a('  L1 -.->|챕터 하나를 다시 씀| S1')
    a("  L3 -.->|납품| P[[프리미어 타임라인]]")
    ...
    a("")
    a("**롱폼 3단계** — 받는 것은 타임코드가 붙은 대본(2단계 산출물), "
      "내놓는 것은 차트만 있는 영상 클립입니다. "
      "자막·타이틀·로고는 2단계에서 이미 들어가므로 렌더에 넣지 않습니다.")
    a("")
    a("**숏폼 1단계** — 롱폼 챕터 하나를 골라 350~560자로 다시 씁니다. "
      "대본을 대신 쓰는 게 아니라 규칙·작성 지시서·검사를 제공합니다.")
    a("")
    ...
    a("   └─ 350~560자로 다시 쓴다     ← 롱폼 전체의 9% 안팎, 약 53~85초")
    a("")
    a("        ① 훅    오늘은 …를 알려드릴게요        20~55자")
    a("        ② 근거  통념 → 하지만 → 손실          110~290자")
    a("        ③ 본론  기준·설정값·순서를 숫자로       150~370자")
    a("        ④ CTA   답 없이 질문으로 넘김          40~135자")
    ...
    a("")
    a("```")
    for sc, pat, ex in q("SELECT scope,pattern,example FROM naming_rule WHERE id<=3 ORDER BY id"):
        a(f"{sc:8} {pat}")
        a(f"{'':8} 예) {ex}")
    a("```")
    a("")
