...
NEXT_STEPS = [
    ...
    (31, "차12 썸네일 팀장 컨펌",
     ...
     "차11 때처럼 문구 수정 요청이 오면 config.json 의 sub/main 만 고쳐 다시 돌리면 된다", "팀장 확인 후"),
]
...
THUMBNAIL_RULES = [
    ...
    (26, "차트가 타이틀 자리를 피해서 그려져야 한다",
     "글자가 앉는 자리는 윗줄 (78,100)~(1306,244) · 아랫줄 (75,250)~(1563,446) 이다",
     "차12 실측 — 아랫줄 이평선+RSI 매매법 이 (75,251,1563,446)",
     "차트를 그릴 때 include 로 가격 눈금 천장에 없는 값을 끼워 캔들을 그 아래로 내린다. "
     "오른쪽 태그(익절·손절)는 x=1563 보다 오른쪽에서 시작하게 봉 번호를 고른다 — "
     "차12 A 는 익절을 90봉에서 91봉으로 한 칸 밀어 상자 왼쪽이 1563 에 맞았다. "
