...
CONSTRAINTS = [
    ...
    ("psd-tools 텍스트", "텍스트를 바꿔도 미리보기에는 예전 글자가 보인다",
     "레이어에 래스터가 캐시돼 있어서다. 파일 안 EngineData 는 새 글자로 들어가 있고 "
     "포토샵은 그걸 읽는다. 열었을 때 옛 글자면 텍스트 레이어를 한 번 클릭하면 다시 그려진다"),
...
NEXT_STEPS = [
    ...
    (14, "썸네일 텍스트 확인", "PSD 를 포토샵에서 열어 타이틀이 새 문구로 보이는지 확인해야 한다. "
     "파일 안 EngineData 에는 새 문구가 들어가 있지만 레이어에 옛 래스터가 캐시돼 있어 "
     "미리보기에는 옛 글자가 보인다. 옛 글자로 열리면 래스터까지 새로 구워 넣어야 한다", "사용자"),
    (15, "썸네일 인물", "차11 은 인물이 없는 회차라 비워 뒀다. 넣으려면 이미지를 받아야 한다. "
     "tools/thumbnail.py 의 --person 과 psdedit 의 '그룹 1' 자리가 이미 있다", "사용자"),
