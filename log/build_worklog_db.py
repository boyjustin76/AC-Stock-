...
ISSUES = [
    ...
    (42, "공용 실행기가 앱을 죽인 뒤에 모달을 찍고 있었다 (09-21, B 실측)", "시간 제한 → taskkill → _fail.png 순서. 앱이 죽으면 모달도 사라져 찍을 것이 없다", "1단계 순서 실수 — 찍기가 죽이기 뒤에 있었다", "찍기(Read-Modal: Win32 글자 + PrintWindow 그림)를 죽이기 앞으로. tools/_com/run.ps1", "_trap_alert 잡으로 모달을 일부러 띄워 _fail.png(420x159 모달 창만)·글자 2줄 확인. 원문 log/inbox/2026-09-21_B_공용실행기_2단계.md", "fixed"),
    ...
    (47, "공용 실행기 bridge 갈래가 'FAIL TIMEOUT'·'OK JOBERR …' 반환을 통과로 냈다 (09-22, D 실측)", "AE 가 alert 에 막히면 실행기 제한 전에 BridgeTalk 이 약 70초 만에 onTimeout → bridge.jsx 가 'FAIL TIMEOUT' 을 돌려주는데 완화 규칙(decision 36: 반환이 비어 있지 않으면 통과)이 exit 0. 잡 안의 예외도 'OK JOBERR …' 로 와서 통과. 모달은 안 찍히고 AE 는 alert 를 띄운 채 남음", "완화 규칙이 '반환 있음' 만 봤다. bridge 는 실패도 문자열로 돌려준다. B 가 고친 '죽이기 전에 찍기' 는 bridge 에선 들어가지도 않는다(BridgeTalk 이 늘 먼저 끝남)", "bridge 는 반환이 OK 로 시작하고 JOBERR 가 없을 때만 답. 시간 초과가 아닌 실패에서도 모달을 보고 찾으면 찍고 그 앱(+포토샵)을 닫는다. Read-Modal 이 모달 가진 프로세스 이름을 돌려준다. tools/_com/run.ps1", "alert 함정 exit 1·71초·426x159 캡처·문구 전부 읽힘, _trap_throw exit 1(모달 없음·창 전부 찍기), 프리미어 probe 통과·save_quit 판정 줄 성공. 원문 log/inbox/2026-09-22_D_공용실행기_bridge갈래_실측.md", "fixed"),
    (48, "E 가 대본 원자료 402 파일(타 채널 자막 146·Pool 174·미공개 대본 docx 15·팀장 피드백)을 public 옆가지에 푸시 (09-22)", "worktree-E_Script 63cd70e·f80f416, 27만 줄. 커밋 메시지: '올리지 않는다는 예전 세션이 스스로 적은 규칙이었다 — 이정찬 결정은 키만 금지'", "기록에 없는 결정을 추론해 실행했다. DB 에는 25(public 유지)·32(키 위치)만 있고 자료 공개 여부는 아무도 정한 적 없다. 총괄도 하루 전 차12 초안 .md 를 본류에 합치며 public 을 붙여 말하지 않았다(선례를 만든 쪽)", "이정찬 결정(decision 38): 원자료·자막·docx 는 저장소 밖. 총괄이 49 파일만 골라 새 커밋으로 얹고(45734ee) .gitignore 에 규칙. E 는 옆가지를 본류로 되돌려 force-push. 규칙: 기록에 없는 결정은 추론하지 않고 이정찬에게 묻는다(decision 29 의 '판단이면 판단이라고' 와 같은 선)", "본류 tree 에 Pool·메이저자막·레퍼런스·방송본·docx 0건(git ls-files 로 확인). 옆가지 정리는 E 몫 — 확인은 다음 fetch 에서", "fixed"),
    (49, "회사 기본폼 표 칸에 대본을 통째로 넣음 — 양식만 읽고 완성본을 안 잼 (E 09-21)", "6단계 표 칸에 400~800자. 회사 완성본은 표 칸 11~81자, 대본은 줄글 란", "빈 양식은 규칙을 안 말해 준다. 완성본(차명09~12)을 재기 전에 썼다. 이정찬 힌트 뒤에 실측", "skeleton_docx.py 가 줄글 란(## 본문 N)을 받도록. 규칙: 새 양식은 완성본 3편 칸별 글자수부터 잰다. 이정찬은 양식과 완성본 1편을 같이 준다", "28c2f62 실측표. 원문 log/inbox/2026-09-22_총괄_E_자료정리_답.md ①", "fixed"),
    (50, "skeleton_docx.py 정규식 OOXML 편집 버그 둘 — <w:t 가 <w:tcPr 까지 잡음, 머리글 소제목에 굵게 지워짐 (E 09-21~22)", "표 라벨이 XML 조각으로 출력, 표 6단계 칸이 통째로 안 채워짐. 소제목 붙이면 w:b 소실. 렌더 확인 불가(PC 에 soffice·pdftoppm·pandoc 없음)", "docx 스킬(풀기→document.xml→묶기→validate)이 있는데 이정찬이 물을 때까지 안 씀. 정규식으로 XML 을 만짐", "태그 경계 못박음 + run 서식 깔기. validate.py --original 을 관문에. 규칙: 새 파일 형식은 스킬·라이브러리 먼저, 자작은 이유를 적고. 렌더 확인은 LibreOffice(next_step 54)", "validate PASSED. 렌더는 미확인. 원문 답 문서 ②", "fixed"),
    (51, "pool_pieces.py 가 조각 54개를 전부 '틀림' — 경로를 두 단계만 올라가 Pool 0편 (E 09-21)", "앵커 대조 결과 0/54", "도구 자체 결함인데 결과를 자료 문제로 읽을 뻔. 도구 시작 검증(정답·오답 견본) 없음", "세 단계로 고침. 규칙: 전부 틀림/전부 맞음은 먼저 도구를 의심. 관문 도구마다 정답 견본 1·오답 견본 1 을 시작 때 돌리고 paths.py 는 Pool 편수 0 이면 exit", "고친 뒤 44/49 맞음·틀림 0. 원문 답 문서 ③", "fixed"),
]
...
DECISIONS = [
    ...
    (36, "공용 실행기 — 판정 줄 없는 옛 잡은 '경고 통과', 대신 수가 늘지 않게 래칫", "판정 줄 있음 → 성공. 없음 + 시간 안 넘김 + bridge 응답 → 통과(경고). 시간 초과·응답 없음 → 실패. tests/test_verdict_lines.py 가 판정 줄 없는 잡 수(09-21 기준 46)를 상한으로 잰다 — 새 잡은 판정 줄 필수, 옛 잡은 손댈 때 넣는다", "제안서는 '없으면 실패' 였으나 잡 대부분이 옛 것이라 그대로면 아무것도 못 돌린다(D). 완화는 받되 문서가 아니라 장치로 묶는다(decision 34). -StrictVerdict 스위치는 안 만든다 — 래칫이 0 이 되면 기본을 엄격으로 바꾼다. 원문 log/inbox/2026-09-21_D_공용실행기_1단계.md. 09-22 보강(issue 47): 'bridge 응답' 은 OK 로 시작하고 JOBERR 가 없는 것만 — FAIL TIMEOUT·JOBERR 는 실패", "BASELINE 이 0 이 될 때 — 그때 '없으면 실패' 로"),
    (37, "Jev — 거르기·분류에 조건부 채택, 순위 판정관은 아님 (E·D 시험 2026-09-21)", "① 순위(둘 중 나은 것) 판정관 채택 안 함 — 13편 69% ② 거름망으로만: 자리 바꿔 두 번 일치 + confidence ≥ 0.7 → 자동, 아니면 사람(13편 중 9 자동·4 사람) ③ 분류(우리 벽 중 어느 것)는 채택 — radar --jev, 상위 2~3 을 보여 주고 사람이 고른다(D-3 11/12) ④ 셈·날짜·이미지·좌표는 계속 코드 ⑤ 미공개 대본은 이정찬 승인 후 전송(09-21 승인)", "E: 한국어 통과, 값 $0.0014, 자리 치우침(constraint 65), S016 에서 틀리며 confidence 0.30·0.04. D: D-3 11/12(conf 맞은 것 0.97·틀린 것 0.87), D-2 는 분류 기준에 달려 승자 없음, D-1 생존 편향(constraint 66). B: '해당 없음' 선택지·허용 목록 → 둘 다 받는다. 원문 log/inbox/2026-09-21_E_Jev_판정관_시험결과.md · log/inbox/2026-09-21_D_Jev시험.md", "팀장 선택 쌍이 쌓여 판정관을 다시 재게 될 때(decision 35) · 자리 치우침이 다음 버전에서 사라질 때. B 시험(09-21 오후) 반영: 셋 다 미달(5/6·9/11·14/20)이나 틀린 9건 중 모델 3·우리 자료 6 — ⑥ 분류 선택지에 TRAPS ①~㉒ 를 넣어야 한다(radar 에 T① 으로 넣음) ⑦ 썸네일 빨강은 자리 규칙, 판정관 자리 아님(thumbnail_rule 22 정정) ⑧ 모달 문구 분류 5/5 정확(0.85~0.98) → 공용 실행기 2단계: 아는 문구면 정해진 대로 기록, 아니면 죽이고 기록, 문 0.8 ⑨ 모델은 무슨 일인지는 읽고 어느 앱인지 자주 틀린다 — 선택지를 앱별로 좁히는 건 미실측. D-4(09-21 저녁) 반영: ⑩ 공용 실행기의 성공/실패 판정에는 안 붙인다 — 규칙 18/20 > Jev 17/20 이고, 수를 견주는 줄에서 0.91·0.86 으로 틀려 confidence 문이 못 거른다(constraint 67). 쓰는 자리 한 줄 규칙: 뜻만으로 갈리면 Jev, 수를 견줘야 갈리면 코드"),
    (38, "저장소 public 유지 + 원자료는 저장소 밖 — 대본 원본·타 채널 자막·Pool·docx 는 안 올린다 (이정찬 결정 2026-09-22)", "private 으로는 못 바꾼다(이정찬). 대신 저장소에는 상황 판단에 필요한 것만: 워크플로우·README·팀장쌍 jsonl·도구 코드(.py)·문서(.md). 원본(Pool 174·타 채널 자막 121·레퍼런스 25·방송본 7·양식)과 .docx·파생 json·백업은 통째로 뺀다. .gitignore 에 같은 규칙. E 옆가지의 두 커밋(63cd70e·f80f416)은 병합하지 않고 남길 49 파일만 새 커밋(45734ee)으로 얹었다 — 지운 것이 본류 역사에 안 들어오게", "E 가 '이정찬 결정은 키만 금지' 라고 읽고 402 파일을 public 옆가지에 푸시했다(issue 48). DB 에 그런 결정은 없었다(25 는 public 유지, 32 는 키 위치). 타 채널 자막 146 파일은 우리 것이 아니고 저작권·평판 위험(총괄 판단). 미공개 대본·팀장 피드백은 회사 내부 자료. 옆가지에 이미 올라간 것은 E 가 자기 가지를 본류로 되돌려 force-push 로 지운다 — GitHub 에 닿지 않는 객체는 한동안 남는다", "저장소를 private 으로 바꿀 수 있게 될 때 · 자료를 저장소에 둬야 할 이유가 생길 때(그때는 private 이 먼저)"),]
...
RUNBOOK = [
    ...
    (20, "오류 레이더 — 벽에 두 번째 부딪히면", "혼자 우회법을 짜기 전에 이미 나온 답을 찾는다 (우리 기록 → Stack Overflow → GitHub)", "python3 tools/radar.py \"<오류 붙여넣기>\"   ·   --jev (Jev 가 우리 벽 중 상위 3, 키 필요)   ·   --file err.txt --repo owner/name --save   ·   --no-web (오프라인)", "스킬 .claude/skills/radar. 총괄 컨테이너는 GitHub 검색 API 가 막혀 MCP search_issues 로. 답은 우리 환경(cp949·ES3·COM)에 맞는지 확인 후 적용"),
...
NEXT_STEPS = [
    ...
    (41, "세션 시작 폴더 — 완료. U-7(로컬 훅·bypass 되돌리기)도 완료 (이정찬+D 09-21)", "저장소 .claude/settings.json(env·훅·스킬)은 세션을 시작한 폴더에서만 읽힌다(공식 settings 문서, D 확인). 지금 B·D·E 는 …\\이정찬\\Claude 에서 시작해 아무것도 안 걸린다. 총괄 판단: 각 세션을 자기 worktree 폴더에서 띄운다(cd <worktree>; claude 또는 claude --worktree <이름>). 그러면 PYTHONUTF8·git_guard·radar 스킬이 자동으로 붙는다. 답이 오기 전엔 D 로컬 훅 유지", "완료"),
    (42, "더원 배너 나머지 문서 둘 — 완료 (09-18, 이정찬이 파일을 건네 총괄이 등재)", "tools/theone/상단배너_공식.md 는 올라왔다(519625b). 같은 폴더에 로컬만 있는 상단배너_로직.md(7.2KB)·상단배너_임베딩분석.md(8.3KB) — 임베딩 채점기 수치(쌍 개수·유사도 분포·홀드아웃)가 거기 있다. 이정찬이 '대본~인덱스~임베딩~로직' 자료를 찾고 있어 둘 다 tools/theone/ 으로", "완료"),
    (43, "MCP 자가점검 스크립트 저장소로 — 완료 (E, 6bb8b0e → tools/mcp_probe.py)", "E 가 scratchpad/mcp_probe.py(initialize → tools/list → tools/call)를 만들어 뒀다. 별 수 믿지 말고 띄워 보는 도구라 전 파트 공용 — tools/mcp_probe.py 로 올려 달라 (radar 와 같은 자리)", "완료"),
    (44, "경로 끝 공백·마침표 가드 — 완료 (B aed48bd · E 6bb8b0e · D 2a7eddd, 이정찬 승인)", "constraint_note 56. 주석에 번호를 적는다. B 의 일러스트레이터 saveAs 는 실측 뒤 반영(미실측 추정)", "완료"),
    (45, "대본→차트장면 파이프라인 — ①② 완료 (D 09-22: 차10 12비트 일괄 촬영 124초 → 콘티.png → AE 프로젝트 20초, 팀장 그림과 8/9 일치), 남은 것 ③④", "①② 완료: batch_capture.py(규칙 먼저, 못 잡은 비트만 Jev 문 0.7 — 선택지를 '대본이 이런 말을 할 때 → 차트는 이렇게' 로 쓰니 0.46→0.97) → conti_sheet.py → ae d1_conti_build. 종목은 BTCUSD M1 우선(팀장 14장 중 9장). 구멍 다섯 잡음(constraint 60 ⑧⑨⑩, 톱니 점수 정규화, 지지선·쓸림 전용 탐색). ③ 다른 회차로 검증 ④ 회사 드라이브에 넣을지 이정찬 판단. 그 다음: 방향(상승/하락) 맞추기, 롱폼 성우 속도 실측(지금은 6.82자/초 추정), B 에게 브랜드 요소(버튼·틀·로고) 얹기 — 아직 넘길 것 없음. 원문 log/inbox/2026-09-22_D_next45_대본→차트장면_일괄촬영·AE세팅.md", "③ D · ④ 이정찬"),
    (46, "어도비 공용 실행기 — 완료 (D 1단계 09-21 낮 · B 2단계 09-21 저녁 · D bridge 갈래 셋 09-22: FAIL/JOBERR 가짜 통과 고침(issue 47), 프리미어 probe·save_quit, 모달 못 찾으면 창 전부 찍기). 복구 창 검사만 53 으로", "run.ps1 네 벌을 tools/_com/run.ps1 하나로 합치면서 ① 앱·문서·프리미어 켜짐 검사 ② 타임아웃→taskkill→실패 기록 ③ 반환값 아닌 판정 줄로 성공 ④ 실패 시 **PrintWindow 로 모달 캡처**(화면 캡처는 가려지면 헛장 — B 09-21, TRAPS ⑨-5)+로그 30줄. 계기: GPT 가 WORKLOG 를 읽고 '진짜 위험은 모달·완료 판정·외부 앱 상태' — DB 로 확인(issue 28~31, TRAPS ⑦⑯). 제안서 log/inbox/2026-09-18_총괄_공용실행기_제안.md", "완료"),
    (47, "팀장 반려 문장 쌍 수동 수집 — E", "반려·첨삭이 올 때마다 고치기 전/후 문장 쌍을 tools/theone/ 에 jsonl 로. 회차·날짜·누가 고쳤나(팀장/전문가/편집) 표시. 30쌍 넘으면 decision 35 다시 본다. 첫 자료가 이미 있다(09-21): 차12 6차 ↔ 이정찬판 INTRO~3구간(팀장 피드백을 이정찬이 적용) — 이 diff 에서 쌍을 뽑으면 된다. 09-22: E 가 뽑았다 — data/대본자료/팀장쌍/pairs.jsonl 88줄(버림/고름/꼴/회차/누가/날짜/출처). 30쌍은 넘었으나 한 회차·한 사람(이정찬)이라 decision 35 재검토는 팀장 것이 섞일 때", "반려가 올 때마다 · 팀장 쌍이 섞이면 decision 35 재검토"),
    ...
    (49, "Jev 판정관 시험 — E 완료 (09-21: 한국어 통과·순위 9/13·자리 치우침·confidence 문 제안 → decision 37)", "① 한국어: 합격 배너 8쌍을 팀장 기준 5개 Score 로 ② 순위: S016 확정본 vs 1안, 8회차 확정본 vs 1판 문구 — 확정본이 이기는 수 ③ 방송본 5쌍 초안 vs 방송본 ④ confidence 분포. 합격선 ②에서 6/8 + ① 정상 → decision 35 갱신·채택, 아니면 external_tool 14 rejected. 미공개 대본은 안 보낸다(이정찬 결정 전). 키는 이정찬 발급 → .secrets TYPESAFE_API_KEY", "완료"),
    ...
    (51, "Jev 시험 — B 완료 (09-21: 5/6·9/11·14/20 미달, 그러나 틀린 9건 중 6건은 우리 자료 — 발견 둘: 선택지에 TRAPS 필요·규칙 22 설명 정정)", "B-1 모달 문구 → 처리 종류 Choice(실제 문구 5~6개 전부) · B-2 썸네일 강조 대상 Choice(규칙 22, 빨강 든 회차 전부) · B-3 오류 분류(20건 중 16). 픽셀·색 코드·캡처 판정은 제외", "완료"),
    (52, "scenes.py 규칙 2 정정 — D", "D 판독: '원리 비트는 민차트' 는 절반만 맞다(2-1(1)은 볼린저+MA). 확실한 건 규칙 3(지표 설명 비트는 그 지표를 켠다)뿐. 심볼·주기도 고정 아님. 팀장 분류 기준 확인 뒤 규칙 2 를 고치거나 뺀다. 원문 log/inbox/2026-09-21_D_Jev시험.md §2", "팀장 기준 확인 → D"),
    (53, "공용 실행기 — AE 충돌 복구 창 감지·즉시 멈춤 — D 완료 (09-22: 시작 전 0.2초·대기 중 5초마다 15.5초, 정상 잡 1.7초 안 막힘)", "강제 종료된 AE 를 다시 띄우면 복구 창(534x365, 대상 앱 #32770, 주창 없음)이 뜨고 잡은 70초 뒤 FAIL TIMEOUT 으로 끝난다. 자동 클릭은 안 한다(constraint 71 — 빗나가면 남의 앱이 뜬다). ① 시작 전 검사에 그 창 감지 → '충돌 복구 창이 떠 있습니다 — 계속을 누르고 다시 부르세요' 로 exit ② 앱을 죽인 실패의 _fail.txt 에 '다음 실행 때 복구 창이 뜹니다' 한 줄. 급하지 않다 — 손댈 때. 완료: modal_text.py --ae-recovery 가 자식 창 뼈대(OS_ViewContainer×2+OS_EditTextContainer, 주창 없음)로 알아본다. 원문 log/inbox/2026-09-22_D_next53_AE복구창_멈춤.md", "완료"),
    (54, "E 대본 작업의 이정찬 몫 넷 — LibreOffice·판본 이름·완성본 동봉·피드백 범위 한 줄 (09-23)", "① E PC 에 LibreOffice → skeleton_docx 결과를 png 로 눈 확인(issue 50) ② docx 판본 이름 차12_vN_<누가>, 복사본·_1 금지(지금 14개) ③ 양식 넘길 때 완성본 1편 동봉(issue 49) ④ 팀장 피드백 받을 때 '몇 구간까지 보셨나' 한 줄. 별도 결정: A/B/C 세 판을 팀장에게 보일지, B(김직선 말투) 판을 쓸지. 원문 log/inbox/2026-09-22_총괄_E_자료정리_답.md 덧붙임", "이정찬"),
]
...
THUMBNAIL_RULES = [
    ...
    (22, "윗줄 부분 빨강",
     ...
     "윗줄 맨 앞 말 = 그 회차가 다루는 문제나 소재. 빨강은 거기 붙는다. 자리 규칙이라 코드가 센다(맨 앞 말). "
     "옛 설명 '셋 다 부정어다' 는 사례 셋을 보고 나중에 붙인 뜻 규칙인데 #10 '변동성 하나로 수익내는' 이 어긋난다 — 변동성은 도구지 부정 대상이 아니다 "
     "(B Jev 시험 09-21: 뜻으로 물으면 3건 중 1건을 확신하며 틀린다). 판정관 자리가 아니다. 긍정문 윗줄에 빨강을 안 쓴 차11 C 판단은 그대로"),
    (23, "빨강은 크기를 데리고 온다",
