...
NEXT_STEPS = [
    ...
    (46, "어도비 공용 실행기 — 시작 전 상태 검사·시간 제한 taskkill·판정 줄 완료 (B·D)", "run.ps1 네 벌을 tools/_com/run.ps1 하나로 합치면서 ① 앱·문서·프리미어 켜짐 검사 ② 타임아웃→taskkill→실패 기록 ③ 반환값 아닌 판정 줄로 성공 ④ 실패 시 캡처+로그 30줄. 계기: GPT 가 WORKLOG 를 읽고 '진짜 위험은 모달·완료 판정·외부 앱 상태' — DB 로 확인(issue 28~31, TRAPS ⑦⑯). 제안서 log/inbox/2026-09-18_총괄_공용실행기_제안.md", "B·D (급한 회차 없을 때 하루)"),
]
