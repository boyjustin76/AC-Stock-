...
REQUESTS = [
    ...
    (21, "모션그래픽 로그를 꼼꼼히 검토하고 렌더 시간을 최소화할 방안을 내라. "
     ...
     "렌더 3.5배 가속 · benchmark 10~17 · decision 12"),
    (22, "썸네일 쪽 코드도 효율성 검토해라. 단, 작업자가 여럿이라(클라우드 A/C · 로컬 B) 조심할 것. "
...
DECISIONS = [
    ...
    (11, "프리미어 MCP", "이 세션에는 설치하지 않는다. 사용자 PC 용으로 보류",
     ...
     "컷 납품 자동화를 시작할 때"),
    (12, "프레임 캡처 경로", "page.screenshot 대신 canvas.toDataURL 로 PNG 를 뽑는다",
...
NEXT_STEPS = [
    (17, "로컬 클로드 푸시 확인 (PC 켜지면 바로)",
     "로컬의 썸네일 작업 기록이 GitHub 에 없다 — 브랜치 1개, 커밋 전부 클라우드 것. "
     "로컬 PC 를 켜면 git log/status 를 보고하게 하고, force push 는 금지시킨다 "
     "(그 사이 렌더 가속 6커밋이 올라가 있다). 안전한 길은 별도 브랜치로 먼저 푸시. "
     "절차는 log/THUMBNAIL-REVIEW.md 0번", "사용자"),
