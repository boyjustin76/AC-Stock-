...
NEXT_STEPS = [
    ...
    (46, "어도비 공용 실행기 — B 2단계 완료(09-21 저녁: 일러·포토샵 direct 갈래, 1단계 버그 셋(issue 42~44), 모달 문구 분류 표→Jev 문 0.8). 남은 셋은 bridge 갈래라 D", "run.ps1 네 벌을 tools/_com/run.ps1 하나로 합치면서 ① 앱·문서·프리미어 켜짐 검사 ② 타임아웃→taskkill→실패 기록 ③ 반환값 아닌 판정 줄로 성공 ④ 실패 시 **PrintWindow 로 모달 캡처**(화면 캡처는 가려지면 헛장 — B 09-21, TRAPS ⑨-5)+로그 30줄. 계기: GPT 가 WORKLOG 를 읽고 '진짜 위험은 모달·완료 판정·외부 앱 상태' — DB 로 확인(issue 28~31, TRAPS ⑦⑯). 제안서 log/inbox/2026-09-18_총괄_공용실행기_제안.md", "D — ① bridge 갈래에서 모달이 포토샵 쪽에 뜨면 Read-Modal 이 못 본다($s.Proc 만) — AE 에서 실제로 나는지 D 가 정한다 ② bridge 의 시간 초과·실패 경로를 새 코드로 한 번(AE 는 성공 경로만 봄) ③ 프리미어 잡 한 번. 원문 log/inbox/2026-09-21_B_공용실행기_2단계.md 끝 절"),
