...
REQUESTS = [
    ...
    (59, "썸네일 3안 다 좌상단 타이틀이 너무 크다. 비율 15~20% 줄여라. "
...
CONSTRAINTS = [
    ...
    ("위스퍼가 단어 안에 침묵을 삼킨다",
     ...
     "tools/cutedit/build_cuts.py 의 SILENCES 가 그 목록"),
    ("컨테이너에서 나가는 파일 한도",
    ...
    ("타이틀 크기는 사람이 정하지 않는다 — 상자가 정한다",
     "차12 썸네일 3안이 다 '좌상단 타이틀이 너무 크다'로 반려 (2026-09-03 이정찬). "
     "규격은 지켰다 — 규칙 4·5 의 고정 높이(윗줄 141 · 아랫줄 194)를 그대로 썼다. "
     "문제는 그게 열 회차 완성본에서 재 온 '수치'라는 것이다. 차12 는 문구가 길어서 "
     "같은 높이인데 폭이 1488px 까지 나갔고 익절 태그와 붙었다. "
     "사용자 지시: 이전 썸네일의 크기를 정량적으로 따르지 말고 맥락(격자박스)으로 파악해라",
     "상한선을 하나 얹었다 — 상자를 먼저 정하고(config.json titleBox), 넓은 줄이 상자 폭을 "
     "넘을 때만 두 줄을 같은 배율로 줄인다. 안 넘으면 예전 회차와 똑같은 고정 크기다. "
     "확대는 안 한다 — 폭에 맞춰 항상 역산하던 옛 컨테이너 방식은 짧은 문구를 거대하게 만든다. "
     "고정 높이 규칙(4·5)을 버린 게 아니라 그 위에 걸어 둔 것이다. 규칙 28"),
]
...
NEXT_STEPS = [
    ...
    (28, "차12 본편 클립 조립 피드백 반영", "사용자가 컷리스트_본편.txt 대로 프리미어에 얹은 뒤 나오는 지적 — "
     ...
     "③ 말 구간 3곳(26.4~56.1 / 92.7~111.3 / 아웃트로)을 정말 비워도 되는지",
     "사용자 조립 후"),
    (29, "차12 인트로+후킹 병합본 조립 피드백 반영", "컷4~6 해석은 총괄 해석으로 확정(2026-09-01, 요청 54) — "
     "랠리 확대까지 완료. 이정찬이 기존 6클립을 지우고 intro-hook.mp4(프레임 166)를 얹은 뒤 나오는 지적 — "
     "호흡·카메라 속도·라벨 타이밍 — 을 받아 scenes/cmg12-cross.build.js 를 고친다. 특히 확인 요청: "
     "① 랠리 리빌 속도(12.9~16.6초 구간, 최대 약 8봉/초)가 여전히 빠른지 ② '익절 기준이 없다'를 "
     "왼쪽 상공(57봉)으로 옮긴 자리가 어색하지 않은지 ③ 상단 잘림 직전 페이드 퇴장(데드크로스 라벨· "
     "매도 태그·가짜 신호, 16.2~18.2초)이 눈에 거슬리지 않는지 ④ (r6) 본편 이월 요소들의 퇴장 시점 — "
     "특히 fail-combo 팬백과 buy-entry 줌인에서 이월 문장이 빠지는 타이밍",
     "이정찬 조립 후"),
    (30, "숏폼 .srt 이관 후속", "E 가 SCRIPT-AGENT-MANUAL.md §8 로 첫 srt 를 뽑으면 결과를 확인한다 "
     "(14자 규칙·절/구 분리·검사 통과 여부). 미결 2건: ① 이미 납품된 차11-4·5 srt 가 새 규칙 기준 "
     "소급 위반(14자 초과 5개·의존명사 시작 2개) — 재발행할지 이정찬 결정 ② E 로컬에 faster-whisper 설치 필요",
     "E 첫 작업 후 / 이정찬 결정"),
    (31, "차12 썸네일 팀장 컨펌",
...
THUMBNAIL_RULES = [
    ...
    (29, "영상 고정 양식에도 같은 상자가 있다",
     "1920x1080 기준 — 좌상단 타이틀 x 0~35% · y 0~20% / 우상단 로고 x 88~100% · y 0~20% / "
     "자막 y 85~100%",
     "차12 본편 프리미어에서 안내선으로 실측 (사용자·Fable 5 총괄)",
     "썸네일 상자(규칙 28)와 숫자는 다르다 — 매체가 다르다. 같은 것은 방식이다: "
     "글자 크기를 회차마다 손으로 정하지 말고 상자를 먼저 정하고 거기 맞춘다. "
     "브랜드 문서는 brand/STYLE.md 2장"),
]
