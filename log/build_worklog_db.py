...
REPO_FILES = {
    ...
    "tools/jev": ("도구", "TypeSafe Jev 얇은 클라이언트 jev.py(choice·score·noul·ask·pick, 키는 .secrets) + D 시험(run_d2·run_d3, 결과 json, 팀장 그림 14장 판독표, 선택지 63). radar --jev 가 jev.py 를 쓴다 (D, 09-21)"),
    ...
    "log/inbox/2026-09-21_D_Jev시험.md": ("기록", "D — Jev 시험: D-3 오류 분류 11/12(상위 2~3 제안), D-2 분류 기준에 따라 6/9↔9/9, D-1 생존 편향으로 미실시(줄 단위 제안), scenes.py 규칙 2 절반만 맞음. §6 자리 치우침 검사 — 분류에선 0 뒤집힘"),
...
NEXT_STEPS = [
    ...
    (46, "어도비 공용 실행기 — D 몫 완료(프리미어 잡 실행·프리미어 떠 있을 때 AE 차단 실측·창 단위 실패 캡처, 09-21 낮), 남은 것 B 2단계(일러·포토샵)", "run.ps1 네 벌을 tools/_com/run.ps1 하나로 합치면서 ① 앱·문서·프리미어 켜짐 검사 ② 타임아웃→taskkill→실패 기록 ③ 반환값 아닌 판정 줄로 성공 ④ 실패 시 **PrintWindow 로 모달 캡처**(화면 캡처는 가려지면 헛장 — B 09-21, TRAPS ⑨-5)+로그 30줄. 계기: GPT 가 WORKLOG 를 읽고 '진짜 위험은 모달·완료 판정·외부 앱 상태' — DB 로 확인(issue 28~31, TRAPS ⑦⑯). 제안서 log/inbox/2026-09-18_총괄_공용실행기_제안.md", "B 2단계 — 실패 캡처는 shot_window.py, 모달 문구 분류는 B-1 결과(5/5, 문 0.8)대로 '아는 문구면 정해진 대로 기록, 아니면 죽이고 기록'"),
